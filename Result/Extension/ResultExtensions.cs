...
public static class ResultExtensions
{
        ...
        Func<T, IResult<TOut>> binder)
    {
        ...
        {
            ISuccessResult<T> success => HelperMethods.Result.Wrap(() => binder(success.Value)),
            IFailureResult failure => HelperMethods.Result.Return<TOut>(failure),
            _ => HelperMethods.Result.Return<TOut>()
        };
    }
    ...
    {
        if (result is not ISuccessResult<T> success) return result;

        return HelperMethods.Result.Wrap(() => filter(success.Value)) switch
        {
            ISuccessResult<bool> filterSuccess => filterSuccess.Value ? success : HelperMethods.Result.Return<T>(),
            IFailureResult filterFailure => HelperMethods.Result.Return<T>(filterFailure),
            _ => HelperMethods.Result.Return<T>()
        };
    }
    ...
    {
        if (result is not ISuccessResult<T> success) return initialState;
    ...
    {
        if (result is not ISuccessResult<T> success) return;
        ...
        Func<T, TOut> mapping)
    {
        return result switch
        {
            ISuccessResult<T> success => HelperMethods.Result.Wrap(() => mapping(success.Value)),
            IFailureResult failure => HelperMethods.Result.Return<TOut>(failure),
            _ => HelperMethods.Result.Return<TOut>()
        };
    }
